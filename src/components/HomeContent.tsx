...
const whyAvm = [
  ...
  },
  {
    ...
    // Overridden at render time by the certificationWording prop (sourced
    // from Sanity's siteSettings) — kept here only as a type-shape fallback.
    desc: "CE, ISO and CDSCO (India's regulatory authority, equivalent to the FDA) certified. Quality is monitored at every stage of manufacturing, from raw material to final product.",
  },
  {
          ...
          <div className="flex flex-col gap-5">
            {whyAvm.map((row) => (
              <WhyCard
                key={row.title}
                title={row.title}
                desc={row.title === "Global Quality Standards" ? certificationWording : row.desc}
              />
            ))}
          </div>
        </div>
